//
// This file was generated by the JavaTM Architecture for XML Binding(JAXB) Reference Implementation, v2.2.7 
// See <a href="http://java.sun.com/xml/jaxb">http://java.sun.com/xml/jaxb</a> 
// Any modifications to this file will be lost upon recompilation of the source schema. 
// Generated on: 2019.07.28 at 05:34:43 PM CEST 
//


package ch.ehi.oereb.schemas.oereb._1_0.extractdata;

import javax.xml.bind.annotation.XmlEnum;
import javax.xml.bind.annotation.XmlEnumValue;
import javax.xml.bind.annotation.XmlType;


/**
 * <p>Java class for LawstatusCode.
 * 
 * <p>The following schema fragment specifies the expected content contained within this class.
 * <p>
 * <pre>
 * &lt;simpleType name="LawstatusCode">
 *   &lt;restriction base="{http://www.w3.org/2001/XMLSchema}string">
 *     &lt;enumeration value="inForce"/>
 *     &lt;enumeration value="runningModifications"/>
 *   &lt;/restriction>
 * &lt;/simpleType>
 * </pre>
 * 
 */
@XmlType(name = "LawstatusCode")
@XmlEnum
public enum LawstatusCodeType {

    @XmlEnumValue("inForce")
    IN_FORCE("inForce"),
    @XmlEnumValue("runningModifications")
    RUNNING_MODIFICATIONS("runningModifications");
    private final String value;

    LawstatusCodeType(String v) {
        value = v;
    }

    public String value() {
        return value;
    }

    public static LawstatusCodeType fromValue(String v) {
        for (LawstatusCodeType c: LawstatusCodeType.values()) {
            if (c.value.equals(v)) {
                return c;
            }
        }
        throw new IllegalArgumentException(v);
    }

}
